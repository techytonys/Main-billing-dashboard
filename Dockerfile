...
COPY attached_asset[s] ./attached_assets
COPY script ./script
COPY client ./client
COPY server ./server
COPY shared ./shared
...
RUN apt-get update -qq && apt-get install -y --no-install-recommends curl ca-certificates libvips42 ffmpeg && rm -rf /var/lib/apt/lists/*
