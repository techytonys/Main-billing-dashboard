...
echo -e "  ${WHITE}[1/5]${RESET} ${BOLD}Pulling latest code${RESET}"
git pull origin main -q
echo -e "        ${CHECK}"
echo ""

echo -e "  ${WHITE}[2/5]${RESET} ${BOLD}Fixing environment${RESET}"
...
echo -e "  ${WHITE}[3/5]${RESET} ${BOLD}Rebuilding app${RESET}"
echo -e "  ${DIM}(this may take a minute)${RESET}"
docker compose build --no-cache app -q 2>&1
echo -e "        ${CHECK}"
echo ""

echo -e "  ${WHITE}[4/5]${RESET} ${BOLD}Restarting services${RESET}"
docker compose up -d --force-recreate app > /dev/null 2>&1
sleep 5
echo -e "        ${CHECK}"
echo ""

echo -e "  ${WHITE}[5/5]${RESET} ${BOLD}Syncing database${RESET}"
