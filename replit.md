...
## Recent Changes
- 2026-02-21: Added client portal server provisioning - clients can spin up their own Linode servers from portal, plan picker (shared plans), region selector, root password + SSH command shown on creation, server cards with IP/SSH copy, support email CTA for help choosing
- 2026-02-21: Added Knowledge Base to client portal - "Help Center" tab with search, category filters, article reading view, renders HTML content from published KB articles
- 2026-02-21: Added Knowledge Base system (/admin/knowledge-base) - Quill rich text editor for article creation, draft/publish workflow, Notion API sync on publish, public /help page with search and category filters, HTML content rendering, article CRUD with categories and sort order
